        ...
        <div class="product__lists-table">
            <appTable :headers="headers" :body="invoice_item">
                ...
                <template #body_action="{item}">
                    <i v-if="inputInvoice.status === 'new' " @click="deletItem(item)" class="deletbtn fa-solid fa-trash-can" ></i> 
                    <i v-if="inputInvoice.status === 'new' " @click="editItem(item)" class="editbtn fa-solid fa-pen-to-square" ></i> 
                 </template>
            </appTable>
            <div class="pagination__action">
                <div class="actions">
                    <router-link v-if="role === 'director' " to="/returned-warhen_d" class="btn1" >
                        <i class="left fa-solid fa-left-long"></i>
                        <span >Back</span>
                    </router-link>
                    <router-link v-if="inputInvoice.status !== 'new' && role !== 'director' " to="/returned-warhen" class="btn1" >
                        <i class="left fa-solid fa-left-long"></i>
                        <span >Back</span>
                    </router-link>
                    <router-link v-if="inputInvoice.status === 'new' && role !== 'director'  " to="/returned-warhen" class="btn">save</router-link>
...
const deletInvoiceItem =()=>{
    http.delete(`/api/warehouse/returned-invoice/${url[4]}/`).then((res)=>{
        if(res.status ===204){
            router.push({name:'returned-warhen'})
...
const confirmInvoice =()=>{
    http.post(`/api/warehouse/returned-invoice/${url[4]}/confirm/`).then((res)=>{
        if(res.status === 200){
            router.push({name:'returned-warhen'})
...
const deletItem=(item)=>{
    http.delete(`/api/warehouse/returned-invoice-item/${item.id}/`).then(res=>{
        if(res.status === 204){
            location.reload()
