<template>
    <section class="warehouse__invoice" >
        ...
        <div class="warehouse__invoice-bar" >
            <button v-if="role !== 'director'" @click="openModal" >Add output invoice</button>
        </div>
            ...
            <appTable :headers="headers" :body="arrved">    
                ...
                <template #body_action="{item}">
                    ...
                    <i @click="deletItem(item)" v-if="item.status ==='new'&& role !=='director' " class="deletbtn fa-solid fa-trash-can" ></i>   
                     <router-link v-if="role === 'director'" :to="{name :'delevered-item_d' , params:{id:item.id} }">
                        <i  class="nextbtn fa-solid fa-eye"></i>
                     </router-link>
                     <router-link v-else :to="{name :'delevered-item' , params:{id:item.id} }">
                        <i  class="nextbtn fa-solid fa-eye"></i>
                     </router-link>
                 </template>
            </appTable>
...
const deletItem =(item)=>{
    http.delete(`/api/warehouse/output-invoice/${item.id}/`).then((res)=>{
        if(res.status ===204){
            location.reload()
