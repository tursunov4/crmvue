<template>
    <appModal v-model="dialog">
        <div class="input_output">
            <h1 v-if="id" >Edit output invoice</h1>
            <h1 v-else>Add output invoice</h1>
               <Form >
                <label for="supplier"  v-if="!id">Select  supplier</label>
                        ...
                        <option v-for="(item , index) in $store.state.client" :value="item.id" >{{item.title}}</option>
                    </select>
                    <label for="description">Description</label>
                    <textarea v-model="forms.description" class="form__text" id="desc" placeholder="Description" ></textarea>
                    <button @click="send" type="button" v-if="id"> Edit input invoice </button>
    ...
    <appModal v-model="dialog2">
        <div class="Password" >
            <div class="Password__change">
                 <h1 v-if="id2">Edit output item</h1>
                <h1 v-else >Add output item</h1>
                 <Form >
                    <label v-if="!id2"  for="product">Select product</label>
                    <Field  v-if="!id2" rules="required" :modelValue="forms.product" v-slot="{ errors }" name="product">
                    ...
                    <button v-if="id2" @click="updateInvoice" type="button">Edit output invoice</button>
                    <button v-else  @click="updateUserPassword" class="buttun__submit" type="button" >Add output invoce</button>
                 </Form>
            </div>
        </div>
    </appModal>
...
const send =()=>{
    if(id){
        http.put(`/api/warehouse/output-invoice/${id.value}/` ,{
...
const sendAdd =()=>{
    http.post('/api/warehouse/output-invoice/' ,{
        client: forms.value.supplier,
        description:forms.value.description
    } ).then((res)=>{
                if(res.status === 201){
                    router.push(`/delevered-item/${res.data.id}`)
                }
         })
}
const updateUserPassword =()=>{
    http.post(`/api/warehouse/output-invoice-item/`, {
...
const updateInvoice =()=>{
    http.put(`/api/warehouse/output-invoice-item/${id2.value}/`, {              
                quantity: forms.value.quantity,
                price: forms.value.price,
            }).then(res => {
                if (res.status === 200) {
                    location.reload()
                    Notification({ text: "Output item updated !!!" }, {type: "success"}
                    )}
                })
}
const getProducts =()=>{
