    ...
    <appModal v-model="dialog2">
        <div class="Password" >
            <div class="Password__change">
                 <h1 v-if="id2">Edit returned invoce</h1>
                <h1 v-else >Add returned invoce</h1>
                 <Form >
                    ...
                    <button v-if="id2" @click="editItem" class="buttun__submit" type="button" >Edit returned invoce</button>
                    <button v-else @click="updateUserPassword" class="buttun__submit" type="button" >Add returned invoce</button>
                 </Form>
            </div>
        </div>
    </appModal>
...
const send =()=>{
    if(id){
        http.put(`/api/warehouse/returned-invoice/${id.value}/` ,{
...
const sendAdd =()=>{
    http.post('/api/warehouse/returned-invoice/' ,{
        client: forms.value.supplier,
        description:forms.value.description
    } ).then((res)=>{
                if(res.status === 201){
                    router.push(`/returned-item/${res.data.id}/`)
                }
         })
}
const updateUserPassword =()=>{
    http.post(`/api/warehouse/returned-invoice-item/`, {
                ...
                if (res.status === 201) {
                    location.reload()
                    Notification({ text: "Returned item updated !!!" }, {type: "success"}
                    )}
                })
}
const editItem =()=>{
    http.put(`/api/warehouse/returned-invoice-item/${id2.value}/`,{
        quantity: forms.value.quantity-0,
        price: forms.value.price-0,
    }).then((res)=>{
        if(res.status==200){
