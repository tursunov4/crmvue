<template>
    <appModal v-model="dialog">
       <section class="group_brand" >
           <h2>Brands</h2>
           <Form @submit="Login">
               <label for="login">Brand Name</label>
               <Field 
                rules="required" name="Brand Name" :modelValue="form.name" v-slot="{errors}" >
               <input type="text" v-model="form.name" id="login" placeholder="Brand Name">
...
const deletGroup =()=>{
   http.delete(`/api/warehouse/brand/${deletId.value}`).then((res)=>{
       if(res.status ===204){
...
const Login =()=>{
   http.post('/api/warehouse/brand/' , {
       ...
       if(res.status ===201){
           Notification({text:'Add Brand'} , {type :'success'})
           location.reload()
       }
   })
}
